...
get_contact_age_weights = function(weightforage=42, weightoffage=1){
  
  all_weights = c()
  for (age in 0:120){
    weights = rep(1, 121)
    if(age <= 17){
      weights = rep(weightoffage, 121)
      weights[age + 1] = weightforage
      weights
    }
...
sample_age_table <- function(prts, cnts, agegroupbreaks, weights=NULL){
  ...
  cnts = merge(cnts, prts_ages, by = 'part_id')
...
getPopdata <- function(country_code="uk", iso3=NULL, year=as.numeric(format(Sys.Date(), "%Y"))){
  popdata <- qs::qread(file.path("../comix/data/unwpp_data.qs"))
...
get_popvec <- function(breaks, year_){
  ## Create labels
...
get_matrix = function(conts, parts, week_choice, breaks, truncate_ = 100, param = "mu", weights=NULL){
  ct_ac = get_age_table(parts = parts, conts = contacts, breaks = breaks, week_choice = week_choice, weights = weights)
...
symetricise_matrix = function(eg, popdata_totals, breaks) {
  eg_props = merge(eg, popdata_totals, by.x="age_group_cont", by.y="age_group")
  ...
  eg_props[,aug_mean_sym:=aug_mean_sym / (props * (length(breaks) - 1))]      
